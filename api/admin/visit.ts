import { serviceSupabase, userFromBearer } from "../_stripe.js";
import { accountEmailKey, cleanPath, cleanReferrer, cleanText, clientCountry, clientIp, jsonResponse } from "../_admin.js";

export async function POST(request: Request): Promise<Response> {
  try {
    const supabase = serviceSupabase();
    if (!supabase) return jsonResponse({ error: "Visit logging is not set up yet." }, 503);
    const body = (await request.json().catch(() => ({}))) as { path?: string; title?: string; referrer?: string };
    const user = await userFromBearer(request);
    const { error } = await supabase.from("site_visits").insert({
      ip: clientIp(request),
      path: cleanPath(body.path),
      title: cleanText(body.title, 80),
      referrer: cleanReferrer(body.referrer),
      country: clientCountry(request),
      user_agent: cleanText(request.headers.get("user-agent"), 400),
      user_id: user?.id ?? null,
      email: accountEmailKey(user?.email) || null,
    });
    if (error) return jsonResponse({ error: "Could not record visit." }, 500);
    return jsonResponse({ ok: true });
  } catch {
    return jsonResponse({ error: "Could not record visit." }, 500);
  }
}
